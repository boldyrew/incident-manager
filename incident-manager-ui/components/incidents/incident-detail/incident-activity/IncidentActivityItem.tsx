...
import {
  ...
  IncidentActivityVisualType,
} from '@/lib/incidentActivityDisplay';
import { IncidentActivity } from '@/types/incident-activity';
import { CheckCircle, Clock, MessageSquare, Pencil, User } from 'lucide-react';
import { IncidentActivityItemDescription } from './IncidentActivityItemDescription';

export interface IncidentActivityItemProps {
  activity: IncidentActivity;
}

function activityIcon(type: IncidentActivityVisualType) {
  switch (type) {
    case 'status_change':
      return (
...
export function IncidentActivityItem({ activity }: IncidentActivityItemProps) {
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
          <p className="text-sm text-foreground">
            ...
            <IncidentActivityItemDescription activity={activity} />
          </p>
